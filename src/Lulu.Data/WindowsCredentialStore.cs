...
{
    private const string GeminiCredentialTarget = "Lulu.Gemini.ApiKey";
    private const string OpenAiCredentialTarget = "Lulu.OpenAI.ApiKey";
    private const string ClaudeCredentialTarget = "Lulu.Claude.ApiKey";
    private const string OpenClawCredentialTarget = "Lulu.OpenClaw.Token";
    private const string AssemblyAiCredentialTarget = "Lulu.AssemblyAI.ApiKey";
    private const string ElevenLabsCredentialTarget = "Lulu.ElevenLabs.ApiKey";
    ...
    private static string? ReadSecret(string target, string provider)
    {
        ...
        if (!CredRead(target, CredTypeGeneric, 0, out var credentialPointer))
        {
            ...
            throw new Win32Exception(error, $"Windows Credential Manager could not read Lulu's {provider} key.");
        }
    ...
    private static void WriteSecret(string target, string provider, string apiKey)
    {
        ...
        try
        {
            ...
            {
                var error = Marshal.GetLastWin32Error();
                throw new Win32Exception(error, $"Windows Credential Manager could not save Lulu's {provider} key.");
            }
        }
        finally
        {
    ...
    private static void DeleteSecret(string target, string provider)
    {
        ...
        if (error != ErrorNotFound)
        {
            throw new Win32Exception(error, $"Windows Credential Manager could not remove Lulu's {provider} key.");
        }
    }
    ...
    private static void EnsureWindows()
    {
        ...
        {
            throw new PlatformNotSupportedException("Lulu's credential store requires Windows.");
        }
    }
