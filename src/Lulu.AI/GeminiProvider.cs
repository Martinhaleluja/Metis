...
public sealed partial class GeminiProvider : IGeminiProvider, IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var payload = GeminiRequestBuilder.BuildGenerateContentJson(
                new GeminiRequest(
                    "This is a Lulu connection diagnostic. Inspect the attached one-pixel image and silent WAV, then reply with exactly the word OK.",
                    DiagnosticPng,
                    DiagnosticWav,
                    "Lulu model diagnostics"));
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (HttpRequestException exception)
        {
            throw new GeminiProviderException(
                GeminiErrorKind.Network,
                "Lulu could not reach Gemini. Check the internet connection, DNS, firewall, or proxy and try again.",
                innerException: exception);
        }
    }
