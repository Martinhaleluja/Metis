...
[assembly: InternalsVisibleTo("Lulu.Tests")]
...
public sealed class DesktopAutomationService : IDesktopAutomationService
{
        ...
        out string error)
    {
        ...
        if (!action.HasCoordinates)
        {
            screenX = 0;
            screenY = 0;
            error = "This action has no coordinate target for Lulu's companion.";
            return false;
        }
        ...
        try
        {
            ...
            if (action.Kind == DesktopActionKind.MovePointer)
            {
                ...
                if (FullControlEnabled && _physicalInput.TryMoveAt(screenX, screenY, out physicalHoverError))
                {
                    return new DesktopActionResult(
                        action,
                        true,
                        FormatSuccess(action, "Moved Lulu with full Windows hover control", screenX, screenY),
                        screenX,
                        screenY);
                }
                ...
                var message = hoverSent
                    ? FormatSuccess(action, "Moved Lulu and sent a cursorless hover", screenX, screenY)
                    : FullControlEnabled
                        ...
                        : $"Moved Lulu to ({screenX}, {screenY}), but the application rejected cursorless hover messages " +
            ...
            if (!_backgroundInput.TryClickAt(action.Kind, screenX, screenY, out var inputError))
            {
                return new DesktopActionResult(
                    ...
                    false,
                    FullControlEnabled
                        ? $"Windows rejected Lulu's full-control and cursorless {Describe(action.Kind)} at ({screenX}, {screenY}). " +
                          $"Full control: {FormatWindowsError(physicalInputError)} Cursorless: {FormatWindowsError(inputError)}"
                        : $"The application at ({screenX}, {screenY}) rejected Lulu's cursorless {Describe(action.Kind)}. " +
    ...
    private DesktopActionResult ExecuteKeyboardOrLaunchAction(DesktopAction action)
    {
        if (!FullControlEnabled)
        {
            return new DesktopActionResult(
                action,
                false,
                "Typing and navigation require Full desktop control in Lulu Setup.");
        }
        ...
        if (!success)
        {
            return new DesktopActionResult(
                ...
                action.Kind switch
                {
                    DesktopActionKind.TypeText => "Windows rejected Lulu's generated typing input.",
        ...
        out string error)
    {
        ...
        if (sourceWidth <= 0 || sourceHeight <= 0)
        {
            screenX = 0;
            screenY = 0;
            error = "The captured desktop has invalid bounds, so Lulu cannot safely position the companion.";
            return false;
        }
