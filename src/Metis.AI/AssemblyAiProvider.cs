...
public sealed class AssemblyAiProvider : IAssemblyAiProvider, IDisposable
{
    ...
    private static readonly IReadOnlyList<string> DefaultModels = ["universal-2"];
    ...
    private async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        catch (HttpRequestException exception)
        {
            throw Error(
                ExternalVoiceErrorKind.Network,
                "Metis could not reach AssemblyAI. Check the internet connection, firewall, DNS, or proxy.",
                innerException: exception);
        }
    }
    ...
    private static JsonDocument ParseJson(string body)
    {
        ...
        catch (JsonException exception)
        {
            throw Error(
                ExternalVoiceErrorKind.EmptyResponse,
                "AssemblyAI returned a response Metis could not read.",
                innerException: exception);
        }
    }
